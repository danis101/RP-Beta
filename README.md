...
- **Background generation:** text, web search and image tools use durable server jobs when automatic summaries are disabled and the chat prompt has no multimodal content. Manual image generation and saved-prompt image regeneration also run on the server. Automatic summaries, vision chat and regeneration from an older user turn retain the browser path. A visible notice identifies where execution happens.

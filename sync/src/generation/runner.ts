...
export class GenerationRunner {
  ...
  start(job: JobRow, openResponse: (signal: AbortSignal) => Promise<Response>): void {
    ...
    void this.run(job, controller, openResponse).finally(() => this.active.delete(key)).catch(error => console.error('[generation]', error))
  }
  ...
  private async run(job: JobRow, controller: AbortController, openResponse: (signal: AbortSignal) => Promise<Response>): Promise<void> {
